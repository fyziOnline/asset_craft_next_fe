...
const downloadOptions = [
    { type: 2, label: 'HTML File' },
    { type: 3, label: 'Zip File' },
    { type: 4, label: 'PDF File' },
]
...
const EditHeader: React.FC<EditHeaderProps> = ({
    ...
    }, [isShowSave, setShowSave]);

    const onDownload = useCallback((type: number) => {
        handleSave(type);
        setShowSave(false);
    }, [handleSave, setShowSave]);
            ...
            <div className='flex gap-4'>
                <div ref={dropdownRef} className='relative w-[150px] bg-white shadow-sm rounded'>
                    ...
                    >
                        ...
                        <span className={`ml-2 transition-transform ${isShowSave ? 'rotate-180' : ''}`}>
                        ...
                        <div className="absolute z-[100] w-full bg-white shadow-sm flex flex-col rounded-b-md px-2 py-1">
                            {downloadOptions.map((opt) => (
                                <button
                                    ...
                                    className="w-full h-10 flex items-center justify-between px-4 hover:bg-[#00A8811A] hover:text-white rounded"
                                >
                                    <div className="text-black text-base font-normal">{opt.label}</div>
