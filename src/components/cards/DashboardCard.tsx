...
const DashboardCard: React.FC<DashboardCardProps> = ({projectName, allProjectDate, totalAssets, underReview, inProgress }) => {
  return (
    <div className="group bg-white hover:bg-green-100 border-2 border-[#D9D9D9] rounded-[15px] w-[280px] h-[230px] p-4 cursor-pointer">
      <h2 className="text-lg leading-none font-bold mb-2 group-hover:text-white">{projectName}:</h2>
      ...
      <div className="flex flex-col items-end mt-4">
        <div className='flex items-baseline'>
          ...
          <p className="text-sm leading-none text-start w-[120px] pl-3 group-hover:text-white">Under Review</p>
        </div>
        <div className='flex items-baseline'>
          <p className="text-4xl font-bold group-hover:text-white">{inProgress}</p>
          <p className="text-sm leading-none text-start w-[120px] pl-3 group-hover:text-white">In Progress</p>
        </div>
      </div>
    </div>
  )
