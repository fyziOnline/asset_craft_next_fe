...
export const useGenerateTemplate = ({ params }: GenerateTemplateProp) => {
    const generateHTML = async (FormData: FormEmailDataProps, Sections: SectionProps[]) => {
        try {
            ...
            if (allSuccess) {
                ...
                return {
                    isSuccess: false,
                    assetVersions: [{
                        htmlGenerated: `<div style="font-size:30px;">Add Section failed, please try again later.</div>`
                    }]
                }
            }
        } catch (error) {
